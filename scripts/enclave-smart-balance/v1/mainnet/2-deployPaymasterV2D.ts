...
async function main() {
  ...
  const paymasterContractFactory = await ethers.getContractFactory("EnclaveSolverPaymasterV2D");
  const paymasterContract = paymasterContractFactory.attach("0xcC6ac1dc190500D12b34509cE24498A96CfFf4B9")
  ...
  //@ts-ignore
  let tx = await usdContract.transfer(paymasterContract.target, "2700000");
  await tx.wait();
}
