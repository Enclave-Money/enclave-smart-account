...
const DefaultsForUserOp = {
  ...
  const getGasFee = async (provider) => {
      ...
      const maxFeePerGas = block.baseFeePerGas
        ? (ethers.toBigInt("2")*block.baseFeePerGas) +ethers.toBigInt(maxPriorityFeePerGas)
...
async function main() {
  ...
  const enclaveSmartAccountAddress =
    "0xa0Dbb12cbb991f91C76E5D1af6e4349f8B8C08f5";
  ...
  const executeEncodedData = enclaveSmartAccount.interface.encodeFunctionData(
    "execute",
    ["0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6", 0, usdcEncodedData]
  );
  ...
  userOp["signature"] = abiCoder.encode(["uint256", "uint256"], [signature.r, signature.s]);
  // userOp["signature"] = sign;
  const pubKey = new Uint8Array([
    2, 140,  54, 246, 159, 254, 197, 122,
   32, 206,  94, 199,  92, 148, 244, 209,
  180, 107, 218, 231,  81, 253, 144, 223,
    3,  93,  36, 244, 251,  84, 169,  99,
  186
]);
const publicKey =[
  ...
  console.log(userOp);
    console.log(p256.verify(signature, message.substring(2), pubKey));
  // return;
  const P256Verifier = await ethers.getContractFactory("P256Verifier");
  const p256Verifier = P256Verifier.attach("0x8A791620dd6260079BF849Dc5567aDC3F2FdC318");
  console.log(await p256Verifier.ecdsa_verify(message, signature.r, signature.s, publicKey))
